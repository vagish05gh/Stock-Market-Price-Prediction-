...
conn = sqlite3.connect('/Users/vrajpatel/Desktop/dd/dh.db')
c = conn.cursor()
...
with st.sidebar:
    ...
    login_submit_button = login_form.form_submit_button('Login')

    # Register form
    register_form = st.form(key="register_form")
    register_form.header('Register')
    username_register = register_form.text_input('Username')
    password_register = register_form.text_input('Password', type='password')
    confirm_password_register = register_form.text_input('Confirm Password', type='password')
    register_submit_button = register_form.form_submit_button('Register')

    if login_submit_button:
        if authenticate(username_login, password_login):
            st.success('Login Successful!')
            # Continue with stock search functionality
        else:
            st.error('Invalid username or password. Please try again.')

    if register_submit_button:
        if password_register == confirm_password_register:
            if register(username_register, password_register):
...
# Continue with the stock market analysis and prediction if user is authenticated
if authenticate(username_login, password_login):
    with st.sidebar:
        stock = option_menu(
        ...
        search_stock_symbol = st.text_input("Enter Stock Symbol for search")

    if search_stock_symbol:
        stock = search_stock_symbol.upper()
    ...
    st.warning('Please login or register to access the stock market analysis and prediction features.')
